...
wlan_process_tx(wlan_private * priv, struct sk_buff *skb)
{
    ...
    u8 insert8023Bytes = 0;
    u8 insertWapiBytes = 0;
    wapi_header *pWAPIHdr = NULL;
    u16 eth_type = 0;
    u16 pkt_type = 0;
    u16 fragThsd = Adapter->FragThsd;
    u8 fragCnt = 1;
    u16 lastPktLen = 0;
    u16 extraSize = 0;
#endif
    ...
    if (Adapter->SecInfo.WAPIEnabled) {
        memcpy(DestAddr, skb->data, ETH_ALEN);
        if ((skb->data[0] != 0xaa) && (skb->data[1] != 0xaa)) {
            skb_pull(skb, 12);
            insert8023Bytes = 6;
        }
        eth_type = *((u16 *) skb->data);
        headlen += insert8023Bytes;
        if (Adapter->SecInfo.WAPI_key_on && (eth_type != WAPI_PKT)) {
            headlen += sizeof(wapi_header);
            insertWapiBytes = sizeof(wapi_header);
            /* check tail fragment */
            if (!IS_GROUP(DestAddr)) {
                extraSize =
                    wlan_get_wapi_fragment_size(Adapter,
                                                (skb->len + insert8023Bytes),
                                                &fragCnt, &fragThsd,
                                                &lastPktLen);
            }
            tailroom = extraSize + MIC_SIZE;
        } else {
            headlen += sizeof(wlan_802_11_header);
            insertWapiBytes = sizeof(wlan_802_11_header);

            pkt_type = *((u16 *) skb->data + 2);
            if (Adapter->cur_wapi_key.Wapi_uni_key.is_key_set
                && (eth_type == WAPI_PKT)
                && (pkt_type == 0x0C01)) {
                Adapter->SecInfo.WAPI_key_on = TRUE;
                PRINTM(CMND, "Tx Key on!\n");
            }
        }
    }
#endif
    ...
    if (Adapter->SecInfo.WAPIEnabled) {
        pWAPIHdr =
            (wapi_header *) (skb->data - insert8023Bytes - insertWapiBytes);
        pLocalTxPD->TxPktOffset = (long) pWAPIHdr - (long) pLocalTxPD;
        pLocalTxPD->TxPktType = PKT_TYPE_802DOT11;
        wlan_prepare_tx_wapi_pkt(Adapter, pWAPIHdr, pLocalTxPD,
                                 skb, DestAddr, insert8023Bytes, eth_type,
                                 fragCnt, fragThsd, lastPktLen);
    }
#endif
...
#ifdef WAPI_DRIVER
    if (Adapter->SecInfo.WAPIEnabled)
        memcpy((u8 *) pLocalTxPD->TxDestAddr, DestAddr, ETH_ALEN);
    else
#endif
