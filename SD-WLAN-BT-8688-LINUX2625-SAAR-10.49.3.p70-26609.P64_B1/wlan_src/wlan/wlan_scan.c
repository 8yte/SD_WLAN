...
#ifdef WAPI
    if (Adapter->SecInfo.WAPIEnabled &&
        (pBSSDesc->pWapiIE &&
         ((*(pBSSDesc->pWapiIE)).IeeeHdr.ElementId == WAPI_IE))) {
        PRINTM(INFO, "Return success for WAPI AP\n");
        LEAVE();
        return index;
    }
#endif /* WAPI */
...
#ifdef WAPI
            Adapter->CurBssParams.BSSDescriptor.pWapiIE = NULL;
            Adapter->CurBssParams.BSSDescriptor.wapiOffset = 0;
#endif
...
#ifdef WAPI
        case WAPI_IE:
            pBSSEntry->pWapiIE = (IEEEtypes_Generic_t *) pCurrentPtr;
            pBSSEntry->wapiOffset = pCurrentPtr - pBSSEntry->pBeaconBuf;
            pBSSEntry->pWapiIE =
                (IEEEtypes_Generic_t *) (pBSSEntry->pBeaconBuf +
                                         pBSSEntry->wapiOffset);
            HEXDUMP("InterpretIE: Resp WAPI_IE", (u8 *) pBSSEntry->pWapiIE,
                    (*(pBSSEntry->pWapiIE)).IeeeHdr.Len +
                    sizeof(IEEEtypes_Header_t));
            break;
#endif /* WAPI */
        }
...
#ifdef WAPI
            if (Adapter->ScanTable[delIdx].pWapiIE) {
                Adapter->ScanTable[delIdx].pWapiIE = (IEEEtypes_Generic_t *)
                    (Adapter->ScanTable[delIdx].pBeaconBuf +
                     Adapter->ScanTable[delIdx].wapiOffset);
            }
#endif
...
#ifdef WAPI
            || Adapter->SecInfo.WAPIEnabled
#endif
            ) {
            if (!Adapter->IsGTK_SET
#ifdef WAPI
                && !Adapter->SecInfo.WAPI_key_on
#endif
                ) {
...
#ifdef WAPI
        if (Adapter->ScanTable[i].pWapiIE) {
            if ((*(Adapter->ScanTable[i].pWapiIE)).IeeeHdr.ElementId == WAPI_IE) {
                pRawData = (u8 *) Adapter->ScanTable[i].pWapiIE;
                memset(&iwe, 0, sizeof(iwe));
                memset(buf, 0, sizeof(buf));
                ptr = buf;
                ptr += sprintf(ptr, "wapi_ie=");

                for (j = 0;
                     j < ((*(Adapter->ScanTable[i].pWapiIE)).IeeeHdr.Len
                          + sizeof(IEEEtypes_Header_t)); j++) {
                    ptr += sprintf(ptr, "%02x", *(pRawData + j));
                }
                iwe.u.data.length = strlen(buf);
...
#ifdef WAPI
                        if (Adapter->ScanTable[adjIdx].pWapiIE) {
                            Adapter->ScanTable[adjIdx].pWapiIE =
                                (IEEEtypes_Generic_t *)
                                (Adapter->ScanTable[adjIdx].pBeaconBuf +
                                 Adapter->ScanTable[adjIdx].wapiOffset);
                        }
#endif
...
#ifdef WAPI
            pNewBeacon->wapiOffset = Adapter->ScanTable[beaconIdx].wapiOffset;
#endif
...
#ifdef WAPI
        if (pNewBeacon->pWapiIE != NULL) {
            pNewBeacon->pWapiIE = (IEEEtypes_Generic_t *)
                (pNewBeacon->pBeaconBuf + pNewBeacon->wapiOffset);
        }
#endif
...
#ifdef WAPI
            pNewBeacon->pWapiIE = NULL;
            pNewBeacon->wapiOffset = 0;
#endif
