 ...
 */
static int
process_requesttpc(int argc, char *argv[])
{
    struct iwreq iwr;
    union
    {
    ...
    if (tpcIoctl.resp.statusCode == 0) {
        printf("tpcrequest:	 txPower(%d), linkMargin(%d), rssi(%d)\n",
               ...
               tpcIoctl.resp.linkMargin, tpcIoctl.resp.rssi);
    } else {
...
/** 
 *	@brief Process Channel Switch Announcement
 *	@param argc		number of arguments
 *	@param argv		A pointer to arguments array	
 *	@return			WLAN_STATUS_SUCCESS--success, otherwise--fail
 */
static int
process_chanswann(int argc, char *argv[])
{
    struct iwreq iwr;
    wlan_ioctl_11h_chan_sw_ann chSwAnnIoctl;
    int ioctl_val, subioctl_val;

    if (get_priv_ioctl("chanswann",
                       &ioctl_val, &subioctl_val) == WLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }

    /* 
     * Channel Switch Announcement expected to be in the following format:
     *
 ...
 */
static int
process_powercap(int argc, char *argv[])
{
    struct iwreq iwr;
    wlan_ioctl_11h_power_cap powerCapIoctl;
    int ioctl_val, subioctl_val;

    if (get_priv_ioctl("powercap",
                       &ioctl_val, &subioctl_val) == WLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
 ...
 */
static int
process_measreq(int argc, char *argv[])
{
    struct iwreq iwr;
    union
    {
 ...
 */
static int
process_setadhocch(int argc, char *argv[])
{
    u32 vals[2];
    struct iwreq iwr;
    int ioctl_val, subioctl_val;

    if (get_priv_ioctl("setadhocch",
                       &ioctl_val, &subioctl_val) == WLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
    ...
    /* band sanity */
    if (strlen(argv[3]) == 2 && (strncmp(argv[3], "bg", 2) == 0)) {
        vals[0] = argv[3][1];
    } else if (strlen(argv[3]) == 1 && (argv[3][0] == 'b' || argv[3][0] == 'a')) {
        vals[0] = argv[3][0];
    } else {
        printf("setadhocch : incorrect band, use a/b/bg.\n");
        exit(1);
    }
 ...
 */
static int
process_getadhocch(int argc, char *argv[])
{
    unsigned int vals[2];
    struct iwreq iwr;
    int ioctl_val, subioctl_val;

    if (get_priv_ioctl("getadhocch",
                       &ioctl_val, &subioctl_val) == WLAN_STATUS_FAILURE) {
        return -EOPNOTSUPP;
    }
 ...
 */
static int
process_sdcmd53r(int argc, char *argv[])
{
    struct ifreq userdata;
    s8 *buf = NULL;
    int addr, mode, blklen, blknum;
    int ioctl_val, subioctl_val;
    ...
    buf = (s8 *) malloc(CMD53BUFLEN);
    if (buf == NULL)
        return -ENOMEM;
    memset(buf, 0, CMD53BUFLEN);        /* CMD53 read */
    if (argc == 8) {
        buf[1] = atoval(argv[3]);       /* func */
    ...
    if (mode) {
        fprintf(stderr, "CMD53r blklen = %d, blknum = %d\n", blklen, blknum);
    } else {
        blklen = 1;
        fprintf(stderr, "CMD53r bytelen = %d\n", blknum);
    }
...
process_txpktstats(int argc, char *argv[])
{
    ...
    for (idx = 0; idx < NELEMENTS(tx_pkt_stats.StatEntry); idx++) {
        tx_pkt_stats.StatEntry[idx].PktInitCnt =
            wlan_le32_to_cpu(tx_pkt_stats.StatEntry[idx].PktInitCnt);
        tx_pkt_stats.StatEntry[idx].PktSuccessCnt =
            wlan_le32_to_cpu(tx_pkt_stats.StatEntry[idx].PktSuccessCnt);
        tx_pkt_stats.StatEntry[idx].TxAttempts =
            wlan_le32_to_cpu(tx_pkt_stats.StatEntry[idx].TxAttempts);
        tx_pkt_stats.StatEntry[idx].RetryFailure =
            wlan_le32_to_cpu(tx_pkt_stats.StatEntry[idx].RetryFailure);
        tx_pkt_stats.StatEntry[idx].ExpiryFailure =
            wlan_le32_to_cpu(tx_pkt_stats.StatEntry[idx].ExpiryFailure);
...
str2bin(char *str, stack_t * stack)
{
    ...
    while ((opstr = getop(str, &first_time)) != NULL) {
        if (isdigit(*opstr)) {
            op.type = TYPE_DNUM;
            dnum = atoi(opstr);
            memcpy((u8 *) op.val, &dnum, sizeof(dnum));
            if (!push(stack, &op)) {
...
process_getfwmem(int argc, char *argv[])
{
    ...
    pGetMem->StartAddr = wlan_cpu_to_le32(StartAddr);
    pGetMem->Len = wlan_cpu_to_le16(Len);
    ...
    while (TRUE) {
        ...
        hexdump("Fw Mem", (u8 *) pGetMem + sizeof(HostCmd_DS_GET_MEM),
                iwr.u.data.length - sizeof(HostCmd_DS_GET_MEM), ' ');
        if (wlan_le32_to_cpu(pGetMem->StartAddr) == 0)
            break;
    }
