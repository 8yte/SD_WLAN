...
bcsp_prepare_pkt(struct bcsp_struct *bcsp, u8 * data, int len, int pkt_type)
{
    ...
    /* Max len of packet: (original len +4(bcsp hdr) +2(crc))*2 (because bytes
       0xc0 and 0xdb are escaped, worst case is when the packet is all made of
       0xc0 and 0xdb :) ) + 2 (0xc0 delimiters at start and end). */
...
bcsp_dequeue(struct hci_uart *hu)
{
    ...
    if ((skb = skb_dequeue(&bcsp->unrel)) != NULL) {
        struct sk_buff *nskb =
            bcsp_prepare_pkt(bcsp, skb->data, skb->len, bt_cb(skb)->pkt_type);
        if (nskb) {
            kfree_skb(skb);
            return nskb;
        } else {
    ...
    /* Now, try to send a reliable pkt. We can only send a reliable packet if
       the number of packets sent but not yet ack'ed is < than the winsize */
    ...
    /* We could not send a reliable packet, either because there are none or
       because there are too many unack'ed pkts. Did we receive any packets we
       have not acknowledged yet ? */

    if (bcsp->txack_req) {
        /* if so, craft an empty ACK pkt and send it on BCSP unreliable channel 
           0 */
    ...
    if (!pass_up) {
        ...
        if (desc != 0 && desc != 1) {
            if (hciextn) {
                ...
                memcpy(skb_push(bcsp->rx_skb, HCI_EVENT_HDR_SIZE), &hdr,
                       HCI_EVENT_HDR_SIZE);
                bt_cb(bcsp->rx_skb)->pkt_type = HCI_EVENT_PKT;
    ...
    while (count) {
        ...
        case BCSP_W4_PKT_START:
            ...
            default:
                ...
                /* Do not increment ptr or decrement count Allocate packet. Max 
                   len of a BCSP pkt= 0xFFF (payload) +4 (header) +2 (crc) */
