 ...
 */
int
wlan_set_wapi_ie(wlan_private * priv, u8 * ie_data_ptr, u16 ie_len)
{
    ...
    if (ie_len) {
        if (ie_len > sizeof(Adapter->Wapi_ie)) {
            PRINTM(INFO, "failed to copy WPA IE, too big \n");
            LEAVE();
            return -EFAULT;
        }
        if (copy_from_user(Adapter->Wapi_ie, ie_data_ptr, ie_len)) {
            PRINTM(INFO, "failed to copy WAPI IE \n");
            LEAVE();
            return -EFAULT;
        }
        Adapter->Wapi_ie_len = ie_len;
        PRINTM(CMND, "Set Wapi_ie_len=%d IE=%#x\n", Adapter->Wapi_ie_len,
               Adapter->Wapi_ie[0]);
        DBG_HEXDUMP(CMD_D, "Wapi_ie", Adapter->Wapi_ie, Adapter->Wapi_ie_len);

        if (Adapter->Wapi_ie[0] == WAPI_IE) {
            Adapter->SecInfo.WAPIEnabled = TRUE;
        }

    } else {
        memset(Adapter->Wapi_ie, 0, sizeof(Adapter->Wapi_ie));
        Adapter->Wapi_ie_len = ie_len;
        PRINTM(INFO, "Reset Wapi_ie_len=%d IE=%#x\n", Adapter->Wapi_ie_len,
               Adapter->Wapi_ie[0]);
        Adapter->SecInfo.WAPI_key_on = FALSE;
#ifdef WAPI_DRIVER
        Adapter->SecInfo.WAPIEnabled = FALSE;
#endif
    }
#ifdef WAPI_DRIVER
    /* Reset wapi key */
    memset(&Adapter->cur_wapi_key, 0, sizeof(&Adapter->cur_wapi_key));
    memset(&Adapter->old_wapi_key, 0, sizeof(&Adapter->old_wapi_key));
#endif
    LEAVE();
    return ret;
}
...
#ifdef WAPI
static int
wlan_set_wapi_key(struct net_device *dev,
                  struct iw_request_info *info,
                  struct iw_point *dwrq, char *extra)
{
...
#ifdef WAPI_DRIVER
    u8 key[32];
    const u8 ucast_def_pn[16] =
        { 0x35, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c,
        0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c, 0x36, 0x5c
    };
    wapi_key *pKey = NULL;
    wapi_key_set key_set;
...
#ifdef WAPI_DRIVER
    memset(&key_set, 0, sizeof(key_set));
    if (memcmp(ext->addr.sa_data, bc_mac, sizeof(bc_mac)) == 0) {
        PRINTM(CMND, "set wapi group key\n");
        pKey = &key_set.Wapi_grp_key;
        memcpy(pKey->macaddr, (u8 *) ext->addr.sa_data, ETH_ALEN);
        memcpy(pKey->seq, ext->tx_seq, IW_ENCODE_SEQ_MAX_SIZE * 2);
        /* init MultiCase Rx PN */
        for (i = 0; i < 16; i++) {
            pKey->RxPN[i] = pKey->seq[15 - i];
        }
    } else {
        PRINTM(CMND, "set wapi unicast key\n");
        pKey = &key_set.Wapi_uni_key;
        /* init Tx PN */
        for (i = 0; i < 4; i++) {
            key_set.WAPI_TxPN[i] = PN_UNICAST_INIT;
        }
        /* init Rx PN */
        memcpy(pKey->RxPN, ucast_def_pn, 16);
        memcpy(pKey->macaddr, (u8 *) ext->addr.sa_data, ETH_ALEN);
        memcpy(pKey->seq, ext->tx_seq, IW_ENCODE_SEQ_MAX_SIZE * 2);
    }
    pKey->keyid = (dwrq->flags & IW_ENCODE_INDEX) - 1;
    memcpy(key, (u8 *) (ext + 1), ext->key_len);
    DBG_HEXDUMP(CMD_D, "set wapi key", (u8 *) key, sizeof(key));
    mkey_expand((u32 *) key, (u32 *) & pKey->rk);
    mkey_expand((u32 *) & key[16], (u32 *) & pKey->mic_rk);
    pKey->is_key_set = TRUE;
    memcpy(&Adapter->old_wapi_key, &Adapter->cur_wapi_key,
           sizeof(wapi_key_set));
    OS_INT_DISABLE(priv, driver_flags);
    if (memcmp(ext->addr.sa_data, bc_mac, sizeof(bc_mac)) == 0) {
        memcpy(&Adapter->cur_wapi_key.Wapi_grp_key, &key_set.Wapi_grp_key,
               sizeof(wapi_key));
    } else {
        memcpy(&Adapter->cur_wapi_key.Wapi_uni_key, &key_set.Wapi_uni_key,
               sizeof(wapi_key));
        memcpy(Adapter->cur_wapi_key.WAPI_TxPN, key_set.WAPI_TxPN,
               sizeof(Adapter->cur_wapi_key.WAPI_TxPN));
    }
    OS_INT_RESTORE(priv, driver_flags);
#else
    memset(&key, 0, sizeof(key));
    memcpy((u8 *) & key.BSSID, (u8 *) ext->addr.sa_data, ETH_ALEN);
    key.KeyIndex = (dwrq->flags & IW_ENCODE_INDEX) - 1;
    key.KeyMaterial[0] = key.KeyIndex;

    if (!Adapter->SecInfo.WAPI_key_on)
        key.KeyMaterial[1] = 1;
    else
        key.KeyMaterial[1] = 0;
    memcpy(&key.KeyMaterial[2], (u8 *) (ext + 1), ext->key_len);
    memcpy(&key.KeyMaterial[ext->key_len + 2], ext->tx_seq,
           IW_ENCODE_SEQ_MAX_SIZE * 2);
    key.KeyLength = ext->key_len + IW_ENCODE_SEQ_MAX_SIZE * 2 + 2;
    if (0 != memcmp(ext->addr.sa_data, bc_mac, sizeof(bc_mac)))
        key.KeyIndex |= 0x40000000;
    else
        Adapter->SecInfo.WAPI_key_on = TRUE;
    ret = wlan_prepare_cmd(priv,
                           HostCmd_CMD_802_11_KEY_MATERIAL,
                           HostCmd_ACT_GEN_SET,
...
#define IW_ENCODE_ALG_SM4   0x20
    if (ext->alg == IW_ENCODE_ALG_SM4) {
        wlan_set_wapi_key(dev, info, dwrq, extra);
        LEAVE();
        return WLAN_STATUS_SUCCESS;
    }
#endif
...
wlan_set_gen_ie_helper(wlan_private * priv, u8 * ie_data_ptr, u16 ie_len)
{
    ...
    if (ie_len == 0) {
        ...
        if ((pVendorIe->ElementId == RSN_IE)
            ...
            Adapter->wps.SessionEnable = FALSE;
#ifdef WAPI
        } else if (pVendorIe->ElementId == WAPI_IE) {
