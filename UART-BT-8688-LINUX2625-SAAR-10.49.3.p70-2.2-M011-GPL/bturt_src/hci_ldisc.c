...
hci_uart_tx_wakeup(struct hci_uart *hu)
{
  ...
  restart:
    ...
    while ((skb = hci_uart_dequeue(hu))) {
        int len;
#ifdef CONFIG_BT_HCIUART_PS
        hexdump("hci_uart Tx", skb->data, skb->len);
#endif
                     ...
                     int count)
{
...
#ifdef CONFIG_BT_HCIUART_PS
    hexdump("hci_uart Rx", (u8 *) data, count);
    ps_start_timer();
#endif
    ...
    if (test_and_clear_bit(TTY_THROTTLED, &tty->flags) && TTY_FUNC->unthrottle)
