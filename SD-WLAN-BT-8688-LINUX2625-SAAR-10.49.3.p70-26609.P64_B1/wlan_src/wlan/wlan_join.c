...
wlan_get_assoc_rsp_ioctl(wlan_private * priv, struct iwreq *wrq) 
{
        ...
        /*  Set the amount to copy back to the application as the minimum of
           the available assoc resp data or the buffer provided by the
           application */ 
...
wlan_associate_to_table_idx(wlan_private * priv, int tableIdx) 
{
        ...
        /*  Check index in table, subtract 1 if within range and call
           association sub-function.  ScanTable[] is 0 based, parameter is 1
           based to conform with IW_ENCODE_INDEX flag parameter passing in
           iwconfig/iwlist */ 
 ...
 */ 
    static int
wlan_cmd_append_wapi_ie(wlan_private * priv, u8 ** ppBuffer) 
{
        ...
        /*  If there is a wapi ie buffer setup, append it to the return
           parameter buffer pointer. */ 
        if (Adapter->Wapi_ie_len) {
        PRINTM(CMND, "append wapi ie %d to %p\n", Adapter->Wapi_ie_len,
                *ppBuffer);
        
            /* Wrap the generic IE buffer with a pass through TLV type */ 
            ieHeader.Type = wlan_cpu_to_le16(MRVL_WAPI_TLV_ID);
        ieHeader.Len = wlan_cpu_to_le16(Adapter->Wapi_ie_len);
            ...
            *ppBuffer += sizeof(ieHeader);
            ...
            memcpy(*ppBuffer, Adapter->Wapi_ie, Adapter->Wapi_ie_len);
        
            /* Increment the return size and the return buffer pointer param */ 
            *ppBuffer += Adapter->Wapi_ie_len;
        retLen += Adapter->Wapi_ie_len;
                          ...
                          void *pdata_buf) 
{
...
#ifdef WAPI
        if (Adapter->SecInfo.WAPIEnabled && Adapter->Wapi_ie_len) {
        wlan_cmd_append_wapi_ie(priv, &pos);
        ...
        /*  Call 11h join API after capability bits are set so adhoc/infra
           11h behavior can be properly triggered.  pos modified if data is
           appended */ 
...
wlan_ret_802_11_ad_hoc(wlan_private * priv, HostCmd_DS_COMMAND * resp) 
{
        ...
        Adapter->MediaConnectStatus = WlanMediaStateConnected;
            ...
            /* Make a copy of current BSSID descriptor, only needed for join
               since the current descriptor is already being used for adhoc
               start */ 
            memcpy(&Adapter->CurBssParams.BSSDescriptor, pBSSDesc,
