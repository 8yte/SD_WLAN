...
#ifdef WAPI_DRIVER
    if (pRxPD->RxPktType == PKT_TYPE_802DOT11) {
        pkt_type = wlan_process_rx_wapi_pkt(priv, skb, pRxPkt, pRxPD);
        switch (pkt_type) {
        case PKT_DROP:
            ret = WLAN_STATUS_FAILURE;
            kfree_skb(skb);
        ...
        case PKT_FRAG_LAST:
            kfree_skb(skb);
            skb = priv->adapter->frag_skb;
            priv->adapter->frag_skb = NULL;
            priv->adapter->last_frag = 0;
            pRxPD = (RxPD *) skb->data;
            break;
        case PKT_NO_FRAG:
            break;
        }
        if ((pkt_type != PKT_NO_FRAG) && (pkt_type != PKT_FRAG_LAST))
            goto done;

        // Adjust pRxPkt
        if ((pRxPD->RxPktOffset + pRxPD->RxPktLength) > skb->len) {
            PRINTM(ERROR,
                   "Wrong wapi rx packet: len=%d,RxPktOffset=%d, RxPktLength=%d\n",
                   skb->len, pRxPD->RxPktOffset, pRxPD->RxPktLength);
            ret = WLAN_STATUS_FAILURE;
            kfree_skb(skb);
            goto done;
